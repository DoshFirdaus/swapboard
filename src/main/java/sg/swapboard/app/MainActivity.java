...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Diagnostics: if the app crashed last time, show the error instead of starting
        final SharedPreferences prefs = getSharedPreferences("diag", MODE_PRIVATE);
        String lastCrash = prefs.getString("crash", null);
        if (lastCrash != null) {
        ...
        Thread.setDefaultUncaughtExceptionHandler((th, ex) -> {
            prefs.edit().putString("crash", Log.getStackTraceString(ex)).commit();
            if (previous != null) previous.uncaughtException(th, ex);
        });
        try {
            setUp(savedInstanceState);
        } catch (Throwable t) {
            showError(Log.getStackTraceString(t));
        }
    }

    private void showError(String details) {
        TextView tv = new TextView(this);
        tv.setText("KakiMart hit an error. Please screenshot this and send it to your developer:\n\n" + details);
        tv.setTextIsSelectable(true);
        tv.setTextSize(13);
        tv.setPadding(40, 160, 40, 160);
