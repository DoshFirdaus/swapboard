...
async function inbox(){
  ...
  app.innerHTML = `<h1>Chats</h1><div class="list">${data.map(c => {
    ...
    return `<a href="#/chat/${c.id}">${img}<div><strong>${esc(other)}</strong><div class="small muted">${esc(c.listings?.title || c.listing_title || "Removed listing")}, ${ago(c.last_message_at)}</div></div></a>`;
  }).join("")}</div>`;
}
